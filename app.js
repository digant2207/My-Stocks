document.addEventListener('DOMContentLoaded', () => {
  ...
  function switchTab(targetId) {
    ...
    if (targetId === 'tab-swot') {
      const stocks = (currentData && (currentData.all_stocks || currentData.top_20_swing)) || [];
      if (stocks.length > 0) {
        const targetSym = (swotTabStockSelect && swotTabStockSelect.value) ? swotTabStockSelect.value : stocks[0].symbol;
        renderSwotDetails(targetSym, stocks);
      }
    }
  }
  ...
  function renderSwotDetails(selectedSym, stocks) {
    if (!stocks || stocks.length === 0) return;
    const stock = stocks.find(s => s.symbol === selectedSym) || stocks[0];
    if (!stock) return;

    if (swotTabStockSelect && swotTabStockSelect.value !== stock.symbol) {
      swotTabStockSelect.value = stock.symbol;
    }

    const currPrice = stock.current_price || 0;
  ...
  function setupSwotTab(stocks) {
    ...
    const currentVal = swotTabStockSelect.value;
    swotTabStockSelect.innerHTML = '';
    stocks.forEach(s => {
    ...
    const targetSym = (currentVal && stocks.some(s => s.symbol === currentVal)) ? currentVal : stocks[0].symbol;
    swotTabStockSelect.value = targetSym;
    renderSwotDetails(targetSym, stocks);
  }

  swotTabStockSelect.addEventListener('change', () => {
    const stocks = (currentData && (currentData.all_stocks || currentData.top_20_swing)) || [];
    renderSwotDetails(swotTabStockSelect.value, stocks);
  });
